...
public:
    ...
    void push(const Eigen::Matrix<T,3,1>& vec)
    {
        N++;
        P += vec * vec.transpose();
        v += vec;
    ...
    {
        Eigen::Matrix<T, 3, 1> center = v / N;
        return P / N - center * center.transpose();
    }

    PointClusterT& operator+=(const PointClusterT& sigv)
    {
        this->P += sigv.P;
        this->v += sigv.v;
        this->N += sigv.N;
    ...
    //void transform(const PointClusterT& sigv, const IMUST& stat)
    //{
    //    N = sigv.N;
    ...
    void transform(const PointClusterT& sigv, const Eigen::Matrix<T,3,3>& R, const Eigen::Matrix<T, 3, 1>& p)
    {
        N = sigv.N;
        v = R * sigv.v + N * p;
        Eigen::Matrix<T, 3, 3> rp = R * sigv.v * p.transpose();
